...
const recordColumns = `id, content_sha256, original_filename, content_type, byte_size, storage_path, language, status, transcript, detected_language, target_language, translated_text, failure_reason, created_at, updated_at, processing_started_at, completed_at`

func scanRecord(row pgx.Row) (r Record, err error) {
	err = row.Scan(&r.ID, &r.ContentSHA256, &r.OriginalFilename, &r.ContentType, &r.ByteSize, &r.StoragePath, &r.Language, &r.Status, &r.Transcript, &r.DetectedLanguage, &r.TargetLanguage, &r.TranslatedText, &r.FailureReason, &r.CreatedAt, &r.UpdatedAt, &r.ProcessingAt, &r.CompletedAt)
	return r, err
}
