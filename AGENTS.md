...
- Go is the primary backend language. Start as a modular monolith; add separate workers only for asynchronous or independently scalable work. The initial Python queue worker invokes OpenAI transcription, translation, and translation-validation APIs; future ML-specific workloads remain separate.
- The MVP supports Somali or English audio with OpenAI auto-detected transcription plus opposite-language text translation, Somali/English text translation, and photo OCR followed by translation. Long-running audio and OCR work must be asynchronous; HTTP webhooks must not wait for it.
...
- Registration codes remain runtime secrets. The temporary local `X-Hadal-Sender` identity shim binds them before WhatsApp is integrated, and will be replaced by the verified webhook sender. Enforce permitted-user 30-second/10-per-day and recruiter 10-second/three-lifetime-then-disabled policy atomically before queueing.
