...
`POST /webhook` validates Meta's `X-Hub-Signature-256`, persists provider message IDs idempotently, and returns immediately. A durable Go intake loop registers senders whose text is exactly one configured four-digit code, queues other text for translation, or authorizes and downloads voice media before queueing the audio worker. Registration codes are classified during authenticated intake and never stored. The provider workers detect Somali, English, or unsupported input. Somali and English are translated into the opposite language; unsupported input receives a Somali/English-only notice, and ambiguous supported text receives a source-language clarification question rather than a guess. Provider workers persist terminal results and outbox replies transactionally and retry transient failures. A Go dispatcher sends pending replies and records Meta's provider message ID.
